var grid = $("#listMyMail").bootstrapTable({
    url:"//courier-api.iskwen.com/api/listMyMail" ,
    ...
    columns: [{
        ...
        formatter : function(value, row, index) {
            if(value == 1){
                return "<button id='type_"+row.id+"' onClick='javascript:typeTip(1,\"type_"+row.id+"\")' class='layui-btn layui-btn-xs btn-normal'>普通</a>";
            }else if(value == 2){
                return "<button id='type_"+row.id+"' onClick='javascript:typeTip(2,\"type_"+row.id+"\")' class='layui-btn layui-btn-xs layui-btn-warm'>html</a>";
            }else if(value == 3){
                return "<button id='type_"+row.id+"' onClick='javascript:typeTip(3,\"type_"+row.id+"\")' class='layui-btn layui-btn-xs layui-btn-danger btn-rounded'>flowWarn</a>";
            }else{
                return "<button id='type_"+row.id+"' onClick='javascript:typeTip(-1,\"type_"+row.id+"\")' class='btn layui-btn-primary btn-rounded btn-xs'>未知</a>";
            }
        }
    },{
        field: 'sender',
        ...
        formatter : function(value, row, index) {
            if(value === 1){
                ...
                return "<button id='state_"+row.id+"' onClick='javascript:stateTip(3,\"state_"+row.id+"\")' class='btn btn-warning btn-rounded btn-xs'>補發</a>";
            }else{
    ...
    },
        {
            ...
            title: '操作',
            formatter : function(value,row,index){
                var result="<button class='layui-btn  layui-btn-xs layui-btn-normal' onClick='javascript:seeMail("+row.id+")'>查看</button>";
                if (row.state==1){
                    result=result+"<button class='layui-btn layui-btn-xs layui-btn-disabled' >重發</button>";
                }else if (row.state==2){
                    result=result+"<button class='layui-btn  layui-btn-xs layui-btn-warm' onClick='javascript:retryMail("+row.id+")'>重發</button>";
                }
                return result+"<button class='layui-btn  layui-btn-xs layui-btn-danger' onClick='javascript:deleteMail("+row.id+")'>刪除</button>";
            }
...
function deleteMail(id){
    if(id==''){
        ...
        showConfirm("溫馨提示","您確定要刪除該記錄嗎？",function(index){
            $.ajax({
                url : "//courier-api.iskwen.com/api/deleteMail",
                type : "post",
                dataType:"json",
                data:"id="+id,
                headers: {'Authorization': getCookie("token")},
...
function retryMail(id){
    if(id==''){
        ...
        showConfirm("溫馨提示","您確定要補發該郵件嗎？",function(index){
            $.ajax({
                url : "//courier-api.iskwen.com/api/retryMail",
                type : "post",
                dataType:"json",
                data:"id="+id,
                headers: {'Authorization': getCookie("token")},
