    ...
    columns: [{
        ...
        return "<button class='btn btn-info btn-rounded btn-xs' data-clipboard-text='"+value+"'>"+value+"</a>";
            ...
            formatter : function(value, row, index) {
                if(value === 1){
                    ...
                    return "<button id='state_"+row.id+"' onClick='javascript:stateTip(2,\"state_"+row.id+"\")' class='btn btn-warning btn-rounded btn-xs'>停用</a>";
                }else if (value === 3){
                    return "<button id='state_"+row.id+"' onClick='javascript:stateTip(3,\"state_"+row.id+"\")' class='btn btn-danger btn-rounded btn-xs'>異常</a>";
                }else{
        ...
        },
        {
            ...
            formatter : function(value,row,index){
                var result="";
                if (row.state===1){
                    result=result+"<button class='layui-btn  layui-btn-xs layui-btn-primary' onClick='javascript:changeSoftwareState("+row.id+",2)'>停用</button>";
                }else if (row.state===2){
                    result=result+"<button class='layui-btn  layui-btn-xs layui-btn' onClick='javascript:changeSoftwareState("+row.id+",1)'>啟用</button>";
                }else if (row.state===3){
                    result=result+"<button class='layui-btn  layui-btn-xs layui-btn' onClick='javascript:changeSoftwareState("+row.id+",1)'>解禁</button>";
                }
                result=result+"<button class='layui-btn  layui-btn-xs layui-btn-normal' onClick='javascript:seeSoftware("+row.id+")'>查看</button>";
                result=result+"<button class='layui-btn  layui-btn-xs layui-btn-warm' onClick='javascript:editSoftware("+row.id+")'>編輯</button>";
                return result+"<button class='layui-btn  layui-btn-xs layui-btn-danger' onClick='javascript:deleteSoftware("+row.id+",\""+row.software_name+"\")'>刪除</button>";
            }
...
function changeSoftwareState(id,newState) {
    if(id===''){
        showTip("請選擇一條記錄");
            ...
            $.ajax({
                url : "//courier-api.iskwen.com/api/changeSoftwareState",
                type : "post",
                dataType:"json",
...
function deleteSoftware(id,softwareName){
    if(id===''){
        showTip("請選擇一條記錄");
    }else{
        showConfirm("溫馨提示","您確定要刪除該應用嗎？",function(index){
            layer.prompt({title: '請輸入\''+softwareName+'\'以確認執行刪除操作'},function(val, index2){
                if(val===softwareName){
                    $.ajax({
                        url : "//courier-api.iskwen.com/api/deleteSoftware",
                        type : "post",
                        dataType:"json",
                        data:"id="+id,
                        headers: {'Authorization': getCookie("token")},
...
function editSoftware(id){
    layer.open({
        ...
        yes : function(index, layero) {
            ...
            $.ajax({
                    url : "//courier-api.iskwen.com/api/updateSoftware?id="+id,
                    type : "post",
                    dataType:"json",
