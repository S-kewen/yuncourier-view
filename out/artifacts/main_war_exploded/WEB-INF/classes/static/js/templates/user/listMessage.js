var grid = $("#listMessage").bootstrapTable({
    url:"//courier-api.iskwen.com:8888/api/listMessage" ,
    ...
    columns: [{
        ...
        formatter : function(value, row, index) {
            if(value == 1){
                return "<button id='state_"+row.news_id+"' onClick='javascript:stateTip(1,\"state_"+row.news_id+"\")' class='btn btn-danger btn-rounded btn-xs'>未讀</a>";
            }else if(value == 2){
                return "<button id='state_"+row.news_id+"' onClick='javascript:stateTip(2,\"state_"+row.news_id+"\")' class='btn btn-primary btn-rounded btn-xs'>已讀</a>";
            }else{
                return "<button id='state_"+row.news_id+"' onClick='javascript:stateTip(-1,\"state_"+row.news_id+"\")' class='btn btn-default btn-rounded btn-xs'>未知</a>";
            }
        }
    },{
        field: 'add_time',
    ...
    },
        {
            ...
            formatter : function(value,row,index){
                var result="";
                if(row.state==1){
                    result="<button class='layui-btn  layui-btn-xs layui-btn-normal' onClick='javascript:readMessage("+row.news_id+")'>確認已讀</button>"
                }
                return result+"<button class='layui-btn  layui-btn-xs layui-btn-danger' onClick='javascript:deleteMessage("+row.news_id+")'>删除</button>";
            }
...
$(document).ready(function() {
    $.ajax({
        url : "//courier-api.iskwen.com:8888/api/getUnreadMessageCount",
        type : "post",
        dataType:"json",
        headers: {'Authorization': getCookie("token")},
...
function deleteMessage(id){
    if(id==''){
        ...
        showConfirm("温馨提示","您确定要删除该信息吗？",function(index){
            $.ajax({
                url : "//courier-api.iskwen.com:8888/api/deleteMessage",
                type : "post",
                dataType:"json",
                data:"id="+id,
                headers: {'Authorization': getCookie("token")},
...
function allReadMessage(){
        showConfirm("温馨提示","您确定要將全部消息設置完已讀嗎？",function(index){
            $.ajax({
                url : "//courier-api.iskwen.com:8888/api/allReadMessage",
                type : "post",
                dataType:"json",
                headers: {'Authorization': getCookie("token")},
                success:function(msg){
...
function readMessage(id){
    if(id==''){
        ...
        showConfirm("温馨提示","您确定要將該信息設已讀吗？",function(index){
            $.ajax({
                url : "//courier-api.iskwen.com:8888/api/readMessage",
                type : "post",
                dataType:"json",
                data:"id="+id,
                headers: {'Authorization': getCookie("token")},
