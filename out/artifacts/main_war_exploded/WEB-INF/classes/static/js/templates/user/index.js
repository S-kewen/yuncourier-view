$(document).ready(function() {
    $.ajax({
        url:"//courier-api.iskwen.com:8888/api/getUserIndexInfo",
        type:"POST",
        dataType: "json",
        headers: {'Authorization': getCookie("token")},
        success:function(msg){
            if(msg.status==0){
                $("#todayMailNum").html(toThousands(msg.todayMailNum));
                $("#allMailNum").html(toThousands(msg.allMailNum));
                $("#todaySmsNum").html(toThousands(msg.todaySmsNum));
                $("#allSmsNum").html(toThousands(msg.allSmsNum));
                $("#allFailNum").html(toThousands(msg.allFailNum));
                if(msg.allFailNum===0){
                    $("#successRate").html("100%");
                }else{
                    $("#successRate").html(((((msg.allSmsNum+msg.allMailNum)-msg.allFailNum)/(msg.allSmsNum+msg.allMailNum))*100).toFixed(2)+"%");
                }

                $("#todayPay").html("$"+toThousands(msg.todayPay.toFixed(2)));
                $("#allPay").html("$"+toThousands(msg.allPay.toFixed(2)));

                $("#username").html(msg.username);
                $("#balance").html(toThousands(msg.balance.toFixed(2))+'<a class="text-navy" style="margin-left:10px;" onclick="goRecharge()">儲值</a>');
    ...
    $.ajax({
        url:"//courier-api.iskwen.com:8888/api/listUserIndexLoginRecord",
        type:"POST",
        dataType: "json",
        data:"state=0&type=1",
        headers: {'Authorization': getCookie("token")},
    ...
    $.ajax({
        url:"//courier-api.iskwen.com:8888/api/listUserIndexMessage",
        type:"POST",
        dataType: "json",
        data:"state=0",
        headers: {'Authorization': getCookie("token")},
        success:function(msg){
            if(msg.list!==undefined){
                ...
                $.each(msg.list, function(index2,json2) {
                    ...
                    newsList=newsList+"<a href=\"#\" class=\"pull-left\"> <img alt=\"image\"class=\"img-circle\" src=\"/images/Sender_admin.jpg\">"+unreadTip+"</a><div class=\"media-body \"><strong>"+json2.title+"</strong><br><small class=\"text-muted\">"+time_Date+" 來自 <strong>"+json2.sender+"</strong></small><div class=\"well\">"+json2.msg+"</div></div>";
                });
                if (newsCount>=50){
...
function modifyInfo(){
    layer.open({
        ...
        yes : function(index, layero) {
            ...
            $.ajax({
                url : "//courier-api.iskwen.com:8888/api/modifyInfo",
                type : "post",
                dataType:"json",
                data:body.serialize(),
                headers: {'Authorization': getCookie("token")},
...
function modifyPassword(){
    layer.open({
        ...
        yes : function(index, layero) {
            ...
            $.ajax({
                url : "//courier-api.iskwen.com:8888/api/modifyPassword",
                type : "post",
                dataType:"json",
